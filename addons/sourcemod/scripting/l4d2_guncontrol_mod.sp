...
static Handle:AmmoTimesCVAR = INVALID_HANDLE;
...
public OnPluginStart()
{
    ...
    AmmoTimesCVAR = CreateConVar("l4d2_guncontrol_ammotimes", "1", " How many times the number of Ammo ", DEFAULT_FLAGS);

    GrenadeResupplyCVAR = CreateConVar("l4d2_guncontrol_allowgrenadereplenish", "0", " Do you allow Players to resupply the Grenade launcher off ammospots ", DEFAULT_FLAGS);
    M60ResupplyCVAR = CreateConVar("l4d2_guncontrol_allowm60replenish", "0", " Do you allow Players to resupply the M60 off ammospots ", DEFAULT_FLAGS);
    GLtoM60TransformCVAR = CreateConVar("l4d2_guncontrol_turnGLintoM60chance", "3", " Turns GL spawns into M60 spawns. Works as chance setting. 1 is FULL chance, 2 is half chance, 3 one third and so on ", DEFAULT_FLAGS);
    IncendAmmoMultiplier = CreateConVar("l4d2_guncontrol_incendammomulti", "1", " Multiplier for Incendiary Ammo Pickup Amount ", DEFAULT_FLAGS);
    ...
    HookConVarChange(AmmoTimesCVAR, CVARChanged);
    ...
    AutoExecConfig(true, "l4d2_guncontrol_mod");
...
UpdateConVars()
{
    SetConVarInt(FindConVar("ammo_assaultrifle_max"), GetConVarInt(AssaultAmmoCVAR) * GetConVarInt(AmmoTimesCVAR));
    SetConVarInt(FindConVar("ammo_smg_max"), GetConVarInt(SMGAmmoCVAR) * GetConVarInt(AmmoTimesCVAR));
    SetConVarInt(FindConVar("ammo_shotgun_max"), GetConVarInt(ShotgunAmmoCVAR) * GetConVarInt(AmmoTimesCVAR));
    SetConVarInt(FindConVar("ammo_autoshotgun_max"), GetConVarInt(AutoShotgunAmmoCVAR) * GetConVarInt(AmmoTimesCVAR));
    SetConVarInt(FindConVar("ammo_huntingrifle_max"), GetConVarInt(HRAmmoCVAR) * GetConVarInt(AmmoTimesCVAR));
    SetConVarInt(FindConVar("ammo_sniperrifle_max"), GetConVarInt(SniperRifleAmmoCVAR) * GetConVarInt(AmmoTimesCVAR));
    SetConVarInt(FindConVar("ammo_grenadelauncher_max"), GetConVarInt(GrenadeLauncherAmmoCVAR) * GetConVarInt(AmmoTimesCVAR));
}
    ...
    while ((ent = FindEntityByClassname(ent, "weapon_grenade_launcher_spawn")) != -1)
    {
        if (prev)
        {
            if (GetRandomInt(1, chance) == 1)
            {
                ...
                SetEntProp(replacement, Prop_Data, "m_iClip1", GetConVarInt(M60AmmoCVAR) * GetConVarInt(AmmoTimesCVAR), 1);
...
public Action:Event_Item_Pickup(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    SetEntProp(targetgun, Prop_Data, "m_iClip1", GetConVarInt(M60AmmoCVAR) * GetConVarInt(AmmoTimesCVAR), 1);
}
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
    if (buttons & IN_USE && !buttondelay[client])
    {
        ...
        if (StrEqual(ent_name, "weapon_ammo_spawn", false) && StrEqual(currentgunname, "weapon_grenade_launcher", false) && GetConVarBool(GrenadeResupplyCVAR))
        {
            ...
            SetEntData(client, iAmmoOffset + GRENADE_LAUNCHER_OFFSET_IAMMO, GetConVarInt(GrenadeLauncherAmmoCVAR) * GetConVarInt(AmmoTimesCVAR));
        ...
        {
            SetEntProp(oldgun, Prop_Data, "m_iClip1", GetConVarInt(M60AmmoCVAR) * GetConVarInt(AmmoTimesCVAR), 1);
        ...
        { //case: Assault rifles
            ...
            maxammo = GetConVarInt(AssaultAmmoCVAR) * GetConVarInt(AmmoTimesCVAR); //get max ammo as set
        }
        ...
        { //case: SMGS
            ...
            maxammo = GetConVarInt(SMGAmmoCVAR) * GetConVarInt(AmmoTimesCVAR); //get max ammo as set
        }
        ...
        { //case: Pump Shotguns
            ...
            maxammo = GetConVarInt(ShotgunAmmoCVAR) * GetConVarInt(AmmoTimesCVAR); //get max ammo as set
        }
        ...
        { //case: Auto Shotguns
            ...
            maxammo = GetConVarInt(AutoShotgunAmmoCVAR) * GetConVarInt(AmmoTimesCVAR); //get max ammo as set
        }
        else if (StrEqual(ent_name, "weapon_hunting_rifle", false))
        { //case: Hunting Rifle
            offsettoadd = HUNTING_RIFLE_OFFSET_IAMMO; //gun type specific offset
            maxammo = GetConVarInt(HRAmmoCVAR) * GetConVarInt(AmmoTimesCVAR); //get max ammo as set
        }
        ...
        { //case: Military Sniper Rifle or CSS Snipers
            ...
            maxammo = GetConVarInt(SniperRifleAmmoCVAR) * GetConVarInt(AmmoTimesCVAR); //get max ammo as set
        }
        else
